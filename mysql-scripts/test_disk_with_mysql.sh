...
for num_threads in ${threads[@]}; do
  ...
  nohup  blktrace -d /dev/vdb -o ${num_threads}_threads_read_write_blktrace.log -w 120 > ${num_threads}_threads_read_write_blk2.log 2>&1 &
  sleep 305
  pkill -9 iostat
  echo "end test:`date`"
  sleep 300
  ...
  nohup  blktrace -d /dev/vdb -o ${num_threads}_threads_write_only_blktrace.log -w 120 > ${num_threads}_threads_write_only_blk2.log 2>&1 &
  sleep 305
  pkill -9 iostat
  echo "end test:`date`"
  sleep 300
  ...
  nohup  blktrace -d /dev/vdb -o ${num_threads}_threads_read_only_blktrace.log -w 120 > ${num_threads}_threads_read_only_blk2.log 2>&1 &
  sleep 305
  pkill -9 iostat
  echo "end test:`date`"
done
